...
from exca_dance.core.constants import ARM_LENGTH, BOOM_LENGTH, BUCKET_LENGTH
...
def test_boom_ninety_degrees_clamps_to_sixty_and_goes_up() -> None:
    fk = ExcavatorFK()
    ...
    assert math.isclose(
        boom_pivot[2], 0.5 + BOOM_LENGTH * math.sin(math.radians(60.0)), abs_tol=1e-9
    )


def test_swing_ninety_rotates_reach_into_positive_y() -> None:
    fk = ExcavatorFK()
    pos = fk.forward_kinematics({JointName.SWING: 90.0})

    for key in ("boom_pivot", "arm_pivot", "bucket_tip"):
        assert abs(pos[key][0]) < 1e-9
        assert pos[key][1] > 0.0


def test_clamp_angles_caps_boom_to_sixty() -> None:
    fk = ExcavatorFK()
    clamped = fk.clamp_angles({JointName.BOOM: 999.0})

    assert clamped[JointName.BOOM] == 60.0
...
def test_top_view_shows_swing_effect() -> None:
    ...
    top_ninety = fk.get_joint_positions_2d_top({JointName.SWING: 90.0})

    assert top_zero[2][1] == 0.0
    assert abs(top_ninety[2][0]) < 1e-9
    assert top_ninety[2][1] > 0.0
